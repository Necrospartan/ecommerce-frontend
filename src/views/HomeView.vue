    ...
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            <MediaCard
                ...
                :key="media.id"
                :media="media"
                @view-details="$emit('view-details', media)"
                @book-media="$emit('book-media', media)"
            />
        </div>
...
const filteredMedia = computed(() => {
    return props.mediaList.filter((media) => {
        const locationMatch =
            !filters.value.location || media.location === filters.value.location
        const typeMatch =
            !filters.value.type || media.type === filters.value.type
        // Date filtering would check availability in a real app
